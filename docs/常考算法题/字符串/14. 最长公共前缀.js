...
// 3. 分治：时间复杂度：O(mlogn)，空间复杂度：O(logn)
var longestCommonPrefix = function(strs) {
  if (strs == null || strs.length == 0) return '';
  // 分治的递归函数
  ...
  // 合并（求解前缀的公共字符长度）
  function merge(prefix1, prefix2) {
    let minLen = Math.min(prefix1.length, prefix2.length);
    for (let i = 0;i < minLen;i++) {
      if (prefix1[i] !== prefix2[i]) {
        return prefix1.slice(0, i);
      }
    }
    return prefix1.slice(0, minLen);
  }
  
  return divideAndConquer(strs, 0, strs.length - 1);
}

// 4. 二分查找：时间复杂度：O(nmlogm)，其中m是字符串数组中的字符串的最小长度，n是字符串的数量；空间复杂度：O(1)
// var longestCommonPrefix = function (strs) {
...
console.log(longestCommonPrefix(["flower", "flow", "flight","flo"])); // "fl"